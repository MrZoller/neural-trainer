...
export default function DatasetView({ datasetId, onNavigate }) {
  ...
  const refresh = useCallback(() => {
    getJSON(`/api/datasets/${datasetId}`).then(setDataset).catch(() => {})
    getJSON(`/api/datasets/${datasetId}/images`).then(setImages).catch(() => {})
    getJSON(`/api/datasets/${datasetId}/report`).then(setReport).catch(() => {})
  }, [datasetId])
  ...
  const classes = report
    ? [...new Set([...Object.keys(report.classes),
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        <button className="text-slate-400 hover:text-slate-200 text-sm"
                onClick={() => onNavigate({ name: 'home' })}>← home</button>
        <h2 className="font-semibold">{dataset?.name}</h2>
        <span className="text-xs text-slate-500">
          {report && `${report.n_labeled} labeled / ${report.n_images} images`}
        </span>
        <span className="flex-1" />
        <button className="bg-slate-800 hover:bg-slate-700 rounded px-3 py-1 text-sm"
      ...
      {progress && (
        <p className="text-sm text-sky-400">Uploading {progress.done}/{progress.total}…</p>
      )}
            ...
            {Object.entries(report.classes).map(([label, c]) => (
              <div key={label}>
                ...
                <span className="text-slate-500 text-xs">
                  {c.images} images · {c.groups} session{c.groups !== 1 ? 's' : ''} ·
                  split {c.train}/{c.val}
                </span>
              </div>
            ))}
          ...
          {report.warnings.length > 0 && (
            <ul className="mt-2 space-y-1">
              {report.warnings.map((w) => (
                <li key={w} className="text-amber-400/90 text-xs">⚠ {w}</li>
              ))}
            </ul>
          )}
      ...
      <div className="bg-slate-900 rounded-xl p-4 border border-slate-800">
        <div className="flex items-center gap-2 flex-wrap mb-3">
          ...
          {classes.map((c) => (
            <button key={c}
              className={`px-2.5 py-0.5 rounded-full text-xs ${selectedLabel === c
                ? 'bg-sky-700 text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}
        ...
        {images.length === 0 ? (
          <p className="text-slate-500 text-sm">
            No images yet. “Import folder” auto-labels from folder names
            (cat/, dog/, …); colored dots mark capture sessions — photos taken
            seconds apart stay on the same side of the train/val split.
          </p>
        ) : (
            ...
            {images.map((img) => (
              <div key={img.id}
                ...
                <img src={`/api/images/${img.id}/file`} alt={img.filename}
                     loading="lazy" className="w-full h-24 object-cover" />
      ...
      <div className="bg-slate-900 rounded-xl p-4 border border-slate-800">
        ...
        <p className="text-xs text-slate-500 mb-3 max-w-2xl">
          Fine-tunes a MobileNetV3 pretrained on ImageNet: the backbone (927k
          weights that already know edges, textures, shapes) stays frozen — only a
          small classifier head learns your classes. That&apos;s why ~30–100 photos
          per class is enough. Starting a run freezes a snapshot of the current
          labels and split, so this run stays reproducible even as the dataset grows.
        </p>
        <div className="flex items-end gap-3 flex-wrap">
          <label className="text-xs text-slate-400">
            epochs
            <input className="block bg-slate-800 rounded px-2 py-1 mt-1 text-sm border border-slate-700 w-20"
                   value={trainCfg.epochs}
                   onChange={(e) => setTrainCfg({ ...trainCfg, epochs: e.target.value })} />
          </label>
          <label className="text-xs text-slate-400">
            learning rate
            <input className="block bg-slate-800 rounded px-2 py-1 mt-1 text-sm border border-slate-700 w-24"
                   value={trainCfg.lr}
                   onChange={(e) => setTrainCfg({ ...trainCfg, lr: e.target.value })} />
          </label>
          <button className="bg-sky-700 hover:bg-sky-600 rounded px-4 py-1.5 text-sm font-medium"
